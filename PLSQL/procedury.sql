...
select ID_NAPRAWY FROM PRACE_NAPRAWCZE p NATURAL JOIN ZAMOWIENIE_NAPRAWY
 z WHERE ID_PRACOWNIKA = 1 AND STATUS NOT LIKE 'naprawiony' GROUP BY ID_NAPRAWY;
...
-- wyswietlenie wymienionych czesci w danej pracy naprawczej
select NAZWA_KATEGORII, NAZWA_PRODUCENTA, MODEL, CENA FROM PRACE_NAPRAWCZE_CZESCI 
NATURAL JOIN CZESCI_ZAMIENNE NATURAL JOIN PRACE_NAPRAWCZE NATURAL JOIN KATEGORIE 
NATURAL JOIN PRODUCENCI WHERE ID_NAPRAWY = 1;
...
SELECT c.ID_CZESCI, MODEL, CENA FROM CZESCI_ZAMIENNE c LEFT JOIN MAGAZYN m ON 
m.id_czesci = c.ID_CZESCI WHERE c.ID_KATEGORII = 1 AND c.ID_PRODUCENTA = 1 AND m.LICZBA_SZTUK > 0
...
select CZESCI_ZAMIENNE.ID_CZESCI, NAZWA_KATEGORII, NAZWA_PRODUCENTA, MODEL, CENA, LICZBA_SZTUK
from (CZESCI_ZAMIENNE NATURAL JOIN PRODUCENCI NATURAL JOIN KATEGORIE) LEFT JOIN MAGAZYN 
...
select ID_NAPRAWY, IMIE as "IMIE_KLIENTA", NAZWISKO as "NAZWISKO_KLIENTA",
DATA_ROZPOCZECIA, DATA_ZAKONCZENIA, STATUS, DO_ZAPLATY  FROM ZAMOWIENIE_NAPRAWY
NATURAL JOIN KLIENCI NATURAL JOIN PLATNOSCI WHERE STATUS = 'naprawiony';
